            ...
            <PopoverTrigger
                ...
                render={
                    <button className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border transition-colors
                        ${current ? current.style : "bg-transparent text-gray-400 border-dashed border-gray-300"}
                    ...
                    {PRIORITY_KEYS.map((key) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center gap-2 w-full px-2 py-1.5 text-xs font-medium rounded-md transition-colors text-slate-700 ${isSelected ? PRIORITY_SELECTED_BG[key] : 'hover:bg-slate-50'}`}
                            >
